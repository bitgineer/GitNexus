...
async function getContextResource(backend: LocalBackend, repoName?: string): Promise<string> {
  ...
  lines.push('  - cross_repo_channels: Cross-repo message flow matching across 10 languages (requires `gitnexus link` first)');
  lines.push('');
...
async function getSetupResource(backend: LocalBackend): Promise<string> {
  ...
  for (const repo of repos) {
    ...
    const lines = [
      ...
      '| `cross_repo_channels` | Cross-repo message flow matching across TS/JS/C#/Python/Java/PHP/Go/Ruby/Swift — which services talk to which via shared channel names |',
      '',
      '## Resources',
      '',
